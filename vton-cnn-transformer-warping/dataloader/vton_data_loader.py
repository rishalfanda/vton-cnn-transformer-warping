"""
vton_data_loader.py
Dataset loader untuk VITON-like structure (support train/test split).
"""
...
# -------------------------
def load_pose_json_to_heatmap(json_path: str, image_size: Tuple[int, int],
                              num_keypoints: int = 18, sigma: int = 6):
    H, W = image_size
...
class VITONDataset(Dataset):
    ...
    def _build_data_list(self):
        # pilih alias sesuai mode
        if self.mode == "train":
            img_alias = self.aliases.get("image_train", [])
            cloth_alias = self.aliases.get("cloth_train", [])
            parse_alias = self.aliases.get("parse_train", [])
            pose_alias = self.aliases.get("openpose_json_train", [])
            mask_alias = self.aliases.get("cloth_mask_train", [])
        else:
            img_alias = self.aliases.get("image_test", [])
            cloth_alias = self.aliases.get("cloth_test", [])
            parse_alias = self.aliases.get("parse_test", [])
            pose_alias = self.aliases.get("openpose_json_test", [])
            mask_alias = self.aliases.get("cloth_mask_test", [])
        ...
        cloth_mask_dir = self._find_folder(mask_alias)

        if image_dir is None or cloth_dir is None:
            raise ValueError("Dataset missing required folders (image/cloth).")
        ...
        for i in range(min_len):
            ...
            if parse_dir:
                fp = parse_dir / f"{Path(img_name).stem}.png"
                if fp.exists():
                    item["parse"] = fp
            if pose_json_dir:
                pj = pose_json_dir / f"{Path(img_name).stem}_keypoints.json"
                if pj.exists():
                    item["pose_json"] = pj
            if cloth_mask_dir:
                cm = cloth_mask_dir / f"{Path(cloth_name).stem}.jpg"
                if cm.exists():
                    item["cloth_mask"] = cm
    ...
    def _load_parse(self, p: Path):
        return np.array(Image.open(p).convert("L")).astype(np.int64)
    ...
    def __getitem__(self, idx):
        ...
        parse = self._load_parse(item["parse"]) if "parse" in item else None
        pose_hm = None
        if "pose_json" in item:
            H, W = self.config["preprocessing"]["image_size"]
            pose_hm = load_pose_json_to_heatmap(
                str(item["pose_json"]), (H, W),
                self.config["preprocessing"].get("pose_keypoints", 18), sigma=6
            )
        ...
        if self.transform:
            if parse is not None:
                ...
                person_t, parse_t = transformed["image"], transformed["mask"]
            else:
                transformed = self.transform(image=person_img)
                person_t, parse_t = transformed["image"], None

            cloth_trans = get_transforms(self.config, mode=self.mode)
            cloth_t = cloth_trans(image=cloth_img)["image"]
        else:
        ...
        if "cloth_mask" in item:
            ...
            cm = cm.resize((self.config["preprocessing"]["image_size"][1],
                            self.config["preprocessing"]["image_size"][0]))
            cm = np.array(cm)
            result["cloth_mask"] = torch.from_numpy(cm).unsqueeze(0).float() / 255.0
        return result
...
def create_dataloaders(config: Dict, batch_size_override: Optional[int] = None):
    ...
    train_ds = VITONDataset(data_root, "train", config, get_transforms(config, "train"))
    val_ds = VITONDataset(data_root, "test", config, get_transforms(config, "val"))
